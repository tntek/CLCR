...
import network, loss, loss_infonce
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        features_test_N_2, _, _ = obtain_nearest_trace_fix_step(features_test_F, feas_all, label_confi)
        features_test_N_2 = features_test_N_2.cuda()

        features_test_B = netB(features_test_F)
        outputs_test_C = netC(features_test_B)

        features_test_N_2_B = netB(features_test_N_2)
        outputs_test_N_2_C = netC(features_test_N_2_B)

        if args.cls_par > 0:
            pred = mem_label[tar_idx]

            classifier_loss_F = nn.CrossEntropyLoss()(outputs_test_C, pred)
            classifier_loss_N_2 = nn.CrossEntropyLoss()(outputs_test_N_2_C, pred)

            classifier_loss = 1.0*classifier_loss_F + 0.7*classifier_loss_N_2
            classifier_loss *= args.cls_par

            loss_nce_1 = lossnce(outputs_test_C, outputs_test_N_2_C)
            classifier_loss = classifier_loss + 0.2 * loss_nce_1 
        ...
        if args.ent:
            softmax_out_F = nn.Softmax(dim=1)(outputs_test_C)
            softmax_out_N_2 = nn.Softmax(dim=1)(outputs_test_N_2_C)
            softmax_out = 1.0 * softmax_out_F + 0.7 * softmax_out_N_2
...
def obtain_label_ts(loader, netF, netB, netC, args, iter_num_update_f):
    ...
    # -------------------------------------------------------------
    cls_val, predict = torch.max(all_output, 1)
    mectrc_mix = (1-cls_val) + 1.0*ent
    len_unconfi_mix = int(ent.shape[0]*(0.6))
    idx_unconfi_mix = mectrc_mix.topk(len_unconfi_mix, largest=True)[-1]
    idx_unconfi_list_mix = idx_unconfi_mix.cpu().numpy().tolist()

    # idx_uu_lst = list(np.where(cls_val > 0.5)[0])
    # -------------------------------------------------------------
...
# 
    ...
    #------------------------------------------------------------------------
    idx_unconfi_list = idx_unconfi_list_mix
    # idx_unconfi_list = idx_unconfi_list_ent
...
def obtain_nearest_trace_fix_step(data_q, data_all, lab_confi):
    ...
    idx_left = np.arange(0, num_sam, 1) # log the elements in data_q whose hypers are not found
    ...
    while flag_is_done == 0:
        ...
        nearest_idx_tmp, idx_last_tmp = get_nearest_sam_idx_trace(data_q_, data_all_, is_mem, ctr_oper, mtx_mem_ignore, nearest_idx_last)
        is_mem = 1
        ...
        if flag_sw_bad == 1:
            ...
            for k in range(num_sam):
                try:
                    _ = list(mtx_log[k, :]).index(1) # check this prediction path contain the credible sample
                    idx_credible_in_path = np.where(mtx_log[k, :] >= 1)[0]
                    idx_credible = mtx_mem_rlt[k, :][idx_credible_in_path]
                    credis_tmp = data_all[idx_credible,:]
                    credis_weights_tmp = F.softmax(torch.FloatTensor(1-(idx_credible_in_path/LN_MEM)), dim=0)
                    credis_weights_tmp = credis_weights_tmp.numpy()
                    credi_mean_tmp = np.average(credis_tmp, axis=0, weights=credis_weights_tmp)
                    data_re_lst.append(credi_mean_tmp)

                except:
                    nearest_unconfi = data_all[mtx_mem_rlt[k, :][0],:]
                    data_re_lst.append(nearest_unconfi)

            data_re = np.vstack(tuple(data_re_lst))
            data_re = torch.FloatTensor(data_re)
            flag_is_done = 1
        else:
            data_q_ = data_all_[nearest_idx_tmp, :]
        ctr_oper += 1

    return data_re, mtx_mem_rlt[:,-1], mtx_mem_rlt[:,-1]  # array
...
def get_nearest_sam_idx_trace(Q, X, is_mem_f, step_num, mtx_ignore, nearest_idx_last_f): # Q、X arranged in format of row-vector
    Xt = np.transpose(X)
    Simo = np.dot(Q, Xt)               
...
def obtain_nearest_idx(data_q, data_all):
    data_q_ = data_q.detach()
    data_all_ = data_all.detach()
    data_q_ = data_q_.cpu().numpy()
    data_all_ = data_all_.cpu().numpy()
...
def get_nearest_sam_idx(Q, X): # Q、X arranged in format of row-vector
    ...
    indices_min = np.argmin(Sim, axis=1)
    indices_row = np.arange(0, Q.shape[0], 1)
    Sim[indices_row, indices_min] = 1000 # let myself out
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_epoch', type=int, default=11, help="max iterations")
    parser.add_argument('--interval', type=int, default=11)
