...
from data_list import ImageList, ImageList_idx_aug_fix
...
def data_load(args): 
    ...
    dsets["source_tr"] = ImageList_idx_aug_fix(tr_txt, transform=image_train())
...
def train_source(args):
    ...
    while iter_num < max_iter:
        try:
            (inputs_source, inputs_source_augs), labels_source, _ = iter_source.next()
        except:
        ...
        inputs_source, labels_source, inputs_source_aug = inputs_source.cuda(), labels_source.cuda(), inputs_source_augs[0].cuda()
        outputs_source = netC(netB(netF(inputs_source)))
        softmax_out = nn.Softmax(dim=1)(outputs_source)
        outputs_source_aug = netC(netB(netF(inputs_source_aug)))
        softmax_out_aug = nn.Softmax(dim=1)(outputs_source_aug)
        
        x_out = softmax_out 
        x_tf_out = softmax_out_aug
        iic_loss = IID_losses.IID_loss(x_out, x_tf_out)
